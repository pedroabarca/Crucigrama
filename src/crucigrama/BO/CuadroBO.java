...
import crucigrama.entidades.Cuadro;
...
public class CuadroBO {
    ...
    public CuadroBO(List listaPistas, JPanel panel, int cat,boolean res) {
        ...
        controlPalabras = new PalabraBO();
       // System.out.println();
         LinkedList<Palabra> lista = controlPalabras.getPalabrasporCategoria(cat);
      
         CargarPistas(listaPistas,lista);
    ...
    private void CargarPistas(List lista,LinkedList<Palabra> list){
        int c = 0;
        lista.removeAll();
        //controlPalabras.getListaPalabras();
        while(c!=list.size()){
            Palabra p = list.get(c);
            lista.add((c+1) + " -"+ p.getPista());
            c++;
        }
    }
